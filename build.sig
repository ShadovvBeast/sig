...
// [sig] Version metadata — CI workflows grep these constants to resolve release versions.
// sig_version struct is the source of truth; sig_version_string is derived for build options.
const sig_version = .{ .major = 0, .minor = 0, .patch = 3, .pre = "dev" };
const sig_version_string = "0.0.3-dev"; // [sig] Must match sig_version above
...
pub fn build(b: *std.Build) !void {
    ...
    exe_options.addOption([:0]const u8, "sig_version", sig_version_string); // [sig]
...
fn discoverSigTools(b: *std.Build) [MAX_TOOLS]?DiscoveredTool {
    ...
    while (it.next(io) catch @panic("failed to read tools directory")) |entry| {
        ...
        var main_file = sub_dir.openFile(io, "main.sig", .{}) catch {
            // No main.sig — skip this directory
            continue;
        };
        main_file.close(io);
        ...
        while (extra_it.next(io) catch @panic("failed to read tool directory")) |extra_entry| {
            ...
            // Import name = "sig_" + stem (e.g., validator.sig → sig_validator)
