...
pub fn build(b: *std.Build) !void {
    // ΓöÇΓöÇ Upstream Section ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    const fmt_include_paths = &.{ "lib", "src", "test", "tools", "build.sig", "build.sig.zon" }; // [sig]
    ...
    // ΓöÇΓöÇ End Upstream Section ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

    // ΓöÇΓöÇ Sig Section ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    const sig_modules = discoverSigModules(b);
    const sig_pbt_files = discoverFiles(b, "test/sig_pbt", "_properties", ".sig");
    const sig_unit_files = discoverFiles(b, "test/sig_unit", "_test", ".sig");
    const sig_bench_files = discoverFiles(b, "test/sig_bench", "_bench", ".sig");
    const sig_tools = discoverSigTools(b);
    ...
    for (sig_tools) |maybe_tool| {
        ...
        for (tool.extra_sources[0..tool.extra_count]) |maybe_extra| {
            ...
            if (all_extras_count >= all_tool_extras.len) {
                std.debug.panic("sig-build-system: too many total tool extras (max {d}). Increase capacity.", .{all_tool_extras.len});
            }
            all_tool_extras[all_extras_count] = extra;
            all_extras_count += 1;
        }
    }

    // Verify harness exists when PBT files are found
    var has_pbt_files = false;
    for (sig_pbt_files) |maybe_file| {
        if (maybe_file != null) {
            has_pbt_files = true;
            break;
        }
    ...
    if (has_pbt_files) {
        const io = b.graph.io;
        var harness_dir = b.build_root.handle.openDir(io, "test/sig_pbt", .{}) catch |err| {
            std.debug.panic("sig-build-system: failed to open test/sig_pbt: {s}", .{@errorName(err)});
        };
        defer harness_dir.close(io);
        var harness_file = harness_dir.openFile(io, "harness.sig", .{}) catch {
            std.debug.panic("sig-build-system: harness.sig not found in test/sig_pbt/ but PBT test files exist", .{});
        };
        harness_file.close(io);
    }

    // ΓöÇΓöÇ test-sig step ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    const sig_test_step = b.step("test-sig", "Run Sig property and unit tests");

    // Register PBT tests (with harness + all imports)
    for (sig_pbt_files) |maybe_file| {
        const file = maybe_file orelse continue;
        const t = b.addTest(.{
            .root_module = b.createModule(.{
                .root_source_file = file.path,
                .target = target,
                .optimize = optimize,
            }),
        });
    ...
    // Register unit tests (without harness, with all imports)
    for (sig_unit_files) |maybe_file| {
        const file = maybe_file orelse continue;
        const t = b.addTest(.{
            .root_module = b.createModule(.{
                .root_source_file = file.path,
    ...
    // ΓöÇΓöÇ bench-sig step ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    const sig_bench_step = b.step("bench-sig", "Run Sig benchmark suite");

    for (sig_bench_files) |maybe_file| {
        const file = maybe_file orelse continue;
        ...
        // Benchmarks get only the sig module import
        for (sig_modules) |maybe_mod| {
            const m = maybe_mod orelse continue;
            if (mem.eql(u8, m.name, "sig")) {
    ...
    // ΓöÇΓöÇ run-{tool} steps ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    for (sig_tools) |maybe_tool| {
        const tool = maybe_tool orelse continue;
        const tool_exe = b.addExecutable(.{
            .name = tool.dir_name,
            .root_module = b.createModule(.{
                .root_source_file = tool.main_path,
                .target = target,
                .optimize = optimize,
            }),
        });
        ...
        for (sig_modules) |maybe_mod| {
            const m = maybe_mod orelse continue;
            if (mem.eql(u8, m.name, "sig")) {
                tool_exe.root_module.addImport("sig", b.createModule(.{
                    .root_source_file = m.path,
                }));
                break;
            }
        }
        // Wire own extras only
        for (tool.extra_sources[0..tool.extra_count]) |maybe_extra| {
            const extra = maybe_extra orelse continue;
            tool_exe.root_module.addImport(extra.name, b.createModule(.{
                .root_source_file = extra.path,
            }));
        }

        // Derive description: "Run {dir_name} tool"
        var desc_buf: [128]u8 = undefined;
        const desc = blk: {
            const prefix = "Run ";
            const suffix = " tool";
            const total = prefix.len + tool.dir_name.len + suffix.len;
            if (total > desc_buf.len) break :blk "Run sig tool";
            @memcpy(desc_buf[0..prefix.len], prefix);
            @memcpy(desc_buf[prefix.len..][0..tool.dir_name.len], tool.dir_name);
            @memcpy(desc_buf[prefix.len + tool.dir_name.len ..][0..suffix.len], suffix);
            break :blk desc_buf[0..total];
        };

        const tool_step = b.step(tool.step_name, desc);
        tool_step.dependOn(&b.addRunArtifact(tool_exe).step);
    }
    // ΓöÇΓöÇ End Sig Section ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
}

// ΓöÇΓöÇ Upstream Helper Functions ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
fn addCmakeCfgOptionsToExe(
    ...
    if (target_result.os.tag.isDarwin()) {
    ...
    if (use_zig_libcxx) {
        ...
        const lib_suffix = if (static) target_result.staticLibSuffix()[1..] else target_result.dynamicLibSuffix()[1..];
        switch (target_result.os.tag) {
            ...
            .openbsd => {
                try addCxxKnownPath(b, cfg, exe, b.fmt("libc++.{s}", .{target_result.dynamicLibSuffix()[1..]}), null, need_cpp_includes);
                try addCxxKnownPath(b, cfg, exe, b.fmt("libc++abi.{s}", .{target_result.dynamicLibSuffix()[1..]}), null, need_cpp_includes);
            },
            .netbsd, .dragonfly => {
...
fn parseConfigH(b: *std.Build, config_h_text: []const u8) ?CMakeConfig {
    ...
    const mappings = [_]struct { prefix: []const u8, field: []const u8 }{
        .{ .prefix = "#define ZIG_CMAKE_BINARY_DIR ", .field = "cmake_binary_dir" },
        .{ .prefix = "#define ZIG_CMAKE_PREFIX_PATH ", .field = "cmake_prefix_path" },
        .{ .prefix = "#define ZIG_CMAKE_STATIC_LIBRARY_PREFIX ", .field = "cmake_static_library_prefix" },
        .{ .prefix = "#define ZIG_CMAKE_STATIC_LIBRARY_SUFFIX ", .field = "cmake_static_library_suffix" },
        .{ .prefix = "#define ZIG_CXX_COMPILER ", .field = "cxx_compiler" },
        .{ .prefix = "#define ZIG_CXX_COMPILER_ARG1 ", .field = "cxx_compiler_arg1" },
        .{ .prefix = "#define ZIG_LLD_INCLUDE_PATH ", .field = "lld_include_dir" },
        .{ .prefix = "#define ZIG_LLD_LIBRARIES ", .field = "lld_libraries" },
        .{ .prefix = "#define ZIG_CLANG_LIBRARIES ", .field = "clang_libraries" },
        .{ .prefix = "#define ZIG_LLVM_LIBRARIES ", .field = "llvm_libraries" },
        .{ .prefix = "#define ZIG_DIA_GUIDS_LIB ", .field = "dia_guids_lib" },
        .{ .prefix = "#define ZIG_LLVM_INCLUDE_PATH ", .field = "llvm_include_dir" },
        .{ .prefix = "#define ZIG_LLVM_LIB_PATH ", .field = "llvm_lib_dir" },
        .{ .prefix = "#define ZIG_SYSTEM_LIBCXX", .field = "system_libcxx" },
    };
...
// ΓöÇΓöÇ Sig Helper Functions ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

// Capacity constants (Task 1.2)
...
const DiscoveredTool = struct {
    dir_name: []const u8,
    step_name: []const u8,
    main_path: std.Build.LazyPath,
    extra_sources: [MAX_TOOL_EXTRAS]?DiscoveredModule,
    extra_count: usize,
};

/// Scan lib/sig/ for *.zig files and add src/sig_diagnostics*.zig modules.
/// Returns a fixed-capacity array of optional DiscoveredModule.
fn discoverSigModules(b: *std.Build) [MAX_MODULES]?DiscoveredModule {
    const io = b.graph.io;
    var result: [MAX_MODULES]?DiscoveredModule = .{null} ** MAX_MODULES;
    var count: usize = 0;

    // Scan lib/sig/ for *.zig files
    var dir = b.build_root.handle.openDir(io, "lib/sig", .{ .iterate = true }) catch {
        // Directory doesn't exist ΓÇö graceful degradation
        return result;
    };
    defer dir.close(io);

    var it = dir.iterateAssumeFirstIteration();
    while (it.next(io) catch @panic("failed to read lib/sig directory")) |entry| {
        ...
        // everything else gets a "sig_" prefix (e.g., io.zig ΓåÆ sig_io)
        ...
        result[count] = .{
            .name = name,
            .path = b.path(b.fmt("lib/sig/{s}", .{entry.name})),
        };
        count += 1;
    }

    // Also add src/sig_diagnostics.zig and src/sig_diagnostics_integration.zig
    if (count >= MAX_MODULES) {
        std.debug.panic("sig-build-system: too many modules (max {d}). Increase MAX_MODULES.", .{MAX_MODULES});
    }
    result[count] = .{
        .name = "sig_diagnostics",
        .path = b.path("src/sig_diagnostics.zig"),
    };
    count += 1;
    ...
    result[count] = .{
        .name = "sig_diagnostics_integration",
        .path = b.path("src/sig_diagnostics_integration.zig"),
    };
    count += 1;
...
/// Scans `dir_path` for files ending with `suffix` + `ext` (e.g., "_properties" + ".sig").
/// Derives name by stripping the extension.
fn discoverFiles(b: *std.Build, dir_path: []const u8, suffix: []const u8, ext: []const u8) [MAX_FILES]?DiscoveredFile {
    const io = b.graph.io;
    var result: [MAX_FILES]?DiscoveredFile = .{null} ** MAX_FILES;
    var count: usize = 0;

    var dir = b.build_root.handle.openDir(io, dir_path, .{ .iterate = true }) catch {
        return result;
    };
    defer dir.close(io);

    var it = dir.iterateAssumeFirstIteration();
    while (it.next(io) catch @panic("failed to read directory")) |entry| {
        if (entry.kind != .file) continue;
        if (!mem.endsWith(u8, entry.name, ext)) continue;

        // Check that the name (without ext) ends with the suffix
        const name_without_ext = entry.name[0 .. entry.name.len - ext.len];
        ...
        result[count] = .{
            .name = name,
            .path = b.path(b.fmt("{s}/{s}", .{ dir_path, entry.name })),
        };
        count += 1;
    }
...
fn discoverSigTools(b: *std.Build) [MAX_TOOLS]?DiscoveredTool {
    const io = b.graph.io;
    var result: [MAX_TOOLS]?DiscoveredTool = .{null} ** MAX_TOOLS;
    var count: usize = 0;

    var dir = b.build_root.handle.openDir(io, "tools", .{ .iterate = true }) catch {
        return result;
    };
    defer dir.close(io);

    var it = dir.iterateAssumeFirstIteration();
    while (it.next(io) catch @panic("failed to read tools directory")) |entry| {
        if (entry.kind != .directory) continue;
        if (!mem.startsWith(u8, entry.name, "sig_")) continue;

        // Check if main.sig exists inside
        var sub_dir = dir.openDir(io, entry.name, .{ .iterate = true }) catch continue;
        defer sub_dir.close(io);

        var main_file = sub_dir.openFile(io, "main.sig", .{}) catch {
            // No main.sig ΓÇö skip this directory
            continue;
        };
        main_file.close(io);

        // Derive step name: replace '_' with '-', prepend "run-"
        var step_buf: [128]u8 = undefined;
        const prefix = "run-";
        if (prefix.len + entry.name.len > step_buf.len) {
            std.debug.panic("sig-build-system: tool name too long: {s}", .{entry.name});
        }
        @memcpy(step_buf[0..prefix.len], prefix);
        @memcpy(step_buf[prefix.len..][0..entry.name.len], entry.name);
        const step_name_raw = step_buf[0 .. prefix.len + entry.name.len];
        // Replace '_' with '-'
        for (step_name_raw) |*c| {
            if (c.* == '_') c.* = '-';
        }
        // Use b.fmt to get a stable string
        const step_name = b.fmt("{s}", .{step_name_raw});

        // Check for duplicate step names
        for (result[0..count]) |maybe_existing| {
            const existing = maybe_existing orelse continue;
            if (mem.eql(u8, existing.step_name, step_name)) {
                std.debug.panic("sig-build-system: duplicate tool step name '{s}'", .{step_name});
            }
        }

        // Discover extra .sig files (not main.sig) as tool-specific imports
        var extras: [MAX_TOOL_EXTRAS]?DiscoveredModule = .{null} ** MAX_TOOL_EXTRAS;
        var extra_count: usize = 0;

        var extra_it = sub_dir.iterateAssumeFirstIteration();
        while (extra_it.next(io) catch @panic("failed to read tool directory")) |extra_entry| {
            if (extra_entry.kind != .file) continue;
            if (!mem.endsWith(u8, extra_entry.name, ".sig")) continue;
            if (mem.eql(u8, extra_entry.name, "main.sig")) continue;

            const stem = std.fs.path.stem(extra_entry.name);
            // Import name = "sig_" + stem (e.g., validator.sig ΓåÆ sig_validator)
            const import_name = b.fmt("sig_{s}", .{stem});
            const extra_path = b.path(b.fmt("tools/{s}/{s}", .{ entry.name, extra_entry.name }));

            if (extra_count >= MAX_TOOL_EXTRAS) {
                std.debug.panic("sig-build-system: too many extra files in {s} (max {d}). Increase MAX_TOOL_EXTRAS.", .{ entry.name, MAX_TOOL_EXTRAS });
            }
            extras[extra_count] = .{
                .name = import_name,
                .path = extra_path,
        ...
        if (count >= MAX_TOOLS) {
            std.debug.panic("sig-build-system: too many tools (max {d}). Increase MAX_TOOLS.", .{MAX_TOOLS});
        }
        result[count] = .{
            .dir_name = b.fmt("{s}", .{entry.name}),
            .step_name = step_name,
            .main_path = b.path(b.fmt("tools/{s}/main.sig", .{entry.name})),
...
fn addSigImports(
    ...
    if (options.include_harness) {
        mod.addImport("harness", b.createModule(.{
            .root_source_file = b.path("test/sig_pbt/harness.sig"),
        }));
    }
